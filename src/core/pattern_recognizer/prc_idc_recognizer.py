...
class ChineseIDCardRecognizer(PatternRecognizer):
    ...
    PATTERNS = [
        Pattern(
            name="id_card_18",
            # 18位身份证
            regex=r"\b[1-9]\d{5}(?:19|20)\d{2}(?:0[1-9]|1[0-2])(?:0[1-9]|[12]\d|3[01])\d{3}(?:\d|X|x)\b",
            score=0.85
        ),
        Pattern(
            name="id_card_15",
            # 15位身份证(老身份证)
            regex=r"\b[1-9]\d{5}\d{2}(?:0[1-9]|1[0-2])(?:0[1-9]|[12]\d|3[01])\d{3}\b",
            score=0.7
        ),
        Pattern(
            name="id_card_with_separator",
            # 带分隔符的身份证号(空格、短横线等)
            regex=r"\b[1-9]\d{5}[\s\-](?:19|20)\d{2}[\s\-]?(?:0[1-9]|1[0-2])[\s\-]?(?:0[1-9]|[12]\d|3[01])[\s\-]?\d{3}[\s\-]?(?:\d|X|x)\b",
            score=0.75
        ),
        Pattern(
            name="id_card_15_with_separator",
            # 带分隔符的15位身份证号
            regex=r"\b[1-9]\d{5}[\s\-]?\d{2}[\s\-]?(?:0[1-9]|1[0-2])[\s\-]?(?:0[1-9]|[12]\d|3[01])[\s\-]?\d{3}\b",
            score=0.65
        )
    ]
