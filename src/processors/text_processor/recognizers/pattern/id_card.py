...
class IDCardRecognizer(PatternRecognizer):
    ...
    PATTERNS = [
        Pattern(
            name="id_card_18",
            # 18位身份证
            regex=r"[1-9]\d{5}(?:19|20)\d{2}(?:0[1-9]|1[0-2])(?:0[1-9]|[12]\d|3[01])"
                  r"\d{3}(?:\d|X|x)",
            score=1
        ),
        Pattern(
        ...
        Pattern(
            ...
            # 带分隔符的身份证号(空格、短横线等)
            regex=r"[1-9]\d{5}[\s\-](?:19|20)\d{2}[\s\-]?(?:0[1-9]|1[0-2])[\s\-]?"
                  r"(?:0[1-9]|[12]\d|3[01])[\s\-]?\d{3}[\s\-]?(?:\d|X|x)",
            score=0.75
        ),
        Pattern(
            name="id_card_15_with_separator",
            # 带分隔符的15位身份证号
            regex=r"[1-9]\d{5}[\s\-]?\d{2}[\s\-]?(?:0[1-9]|1[0-2])[\s\-]?"
                  r"(?:0[1-9]|[12]\d|3[01])[\s\-]?\d{3}",
            score=0.65
        )
    ]
    ...
    def validate_result(self, pattern_text: str) -> bool:
        ...
        if length == 18:
            if not (sanitized_value[:-1].isdigit() and 
                   (sanitized_value[-1].isdigit() or sanitized_value[-1] == 'X')):
                return False
        else:  # 15位
