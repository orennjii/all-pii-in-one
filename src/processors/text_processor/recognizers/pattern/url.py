...
class URLRecognizer(PatternRecognizer):
    ...
    PATTERNS = [
        Pattern(
            ...
            # 带协议的完整URL
            regex=r"\b(?:https?|ftp):\/\/[-a-zA-Z0-9@:%._\+~#=]{1,256}\.[a-zA-Z0-9()]"
                  r"{1,6}\b(?:[-a-zA-Z0-9()@:%_\+.~#?&//=]*)",
            score=0.85
        ),
        Pattern(
            name="url_with_www",
            # 以www开头但无协议的URL
            regex=r"www\.[-a-zA-Z0-9@:%._\+~#=]{1,256}\.[a-zA-Z0-9()]{1,6}"
                  r"(?:[-a-zA-Z0-9()@:%_\+.~#?&//=]*)",
            score=0.8
        ),
        Pattern(
            name="simple_domain",
            # 简单域名格式
            regex=r"\b[-a-zA-Z0-9@:%._\+~#=]{1,256}\.[a-zA-Z0-9()]{1,6}\b"
                  r"(?:[-a-zA-Z0-9()@:%_\+.~#?&//=]*)",
            score=0.7
        ),
        Pattern(
            name="ip_address_url",
            # IP地址形式的URL
            regex=r"\b(?:https?:\/\/)?(?:(?:25[0-5]|2[0-4][0-9]|[01]?[0-9][0-9]?)\.){3}"
                  r"(?:25[0-5]|2[0-4][0-9]|[01]?[0-9][0-9]?)(?::\d{1,5})?\b"
                  r"(?:[-a-zA-Z0-9()@:%_\+.~#?&//=]*)",
            score=0.75
        ),
        Pattern(
            name="cjk_domain",
            # 包含中日韩文字的URL(中文域名)
            regex=r"\b(?:https?:\/\/)?[\u4e00-\u9fff\u3040-\u309f\u30a0-\u30ff\u3400-\u4dbf]+"
                  r"\.(?:[a-z]{2,}|[\u4e00-\u9fff]+)\b(?:[-a-zA-Z0-9()@:%_\+.~#?&//=]*)",
            score=0.8
        )
    ]
    ...
    def validate_result(self, pattern_text: str) -> bool:
        ...
        # 检查URL是否包含有效的顶级域名
        tld_pattern = r'\.(com|net|org|gov|edu|io|ai|app|co|info|cn|com\.cn|org\.cn|'
        tld_pattern += r'edu\.cn|gov\.cn|biz|top|xyz|site|online|tech|dev|me|tv|cc)($|\/|\?|#)'
        if not re.search(tld_pattern, sanitized_value, re.IGNORECASE):
            # 如果没有匹配常见顶级域名，但格式正确也可以接受
            # 例如一些不太常见的国家顶级域名或新顶级域名
            domain_pattern = r'(https?:\/\/|www\.)?([a-zA-Z0-9][-a-zA-Z0-9]{0,62}\.)+[a-zA-Z]'
            domain_pattern += r'{2,}($|\/|\?|#)'
